...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QTabWidget, QWidget, 
                             ...
                             QMessageBox, QFrame, QSlider, QHBoxLayout, QVBoxLayout)
from PyQt5.QtGui import QIcon, QFont, QColor, QPalette
from PyQt5.QtCore import Qt, QSize, QTimer, pyqtSignal, QThread  # تمت إضافة QThread
...
# تم تغيير الفئة لترث من QThread بدلاً من threading.Thread
class TrackerThread(QThread):
    update_status = pyqtSignal(str)
    
    def __init__(self, target_color, threshold, sensitivity, parent=None):
        super().__init__(parent)  # استدعاء منشئ QThread
    ...
    def stop(self):
        """إيقاف الخيط بأمان"""
        self.running = False
        self.wait(500)  # انتظار حتى 500 مللي ثانية
...
class AimAssistApp(QMainWindow):
    def __init__(self):
        ...
        # Create tracker thread - note: added parent=self
        self.tracker = TrackerThread(self.target_color, self.threshold, self.sensitivity, parent=self)
    ...
    def select_target_color(self):
        ...
        def mouse_callback(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                self.update_color_preview()
                self.tracker.set_color(self.target_color)
    ...
    def update_sensitivity(self, value):
        ...
        self.tracker.set_sensitivity(self.sensitivity)
    ...
    def reset_settings(self):
        ...
        self.sensitivity_label.setText(f"{self.sensitivity:.2f}")
        self.tracker.set_threshold(self.threshold)
        self.tracker.set_sensitivity(self.sensitivity)
    ...
    def closeEvent(self, event):
        ...
        if self.tracker.isRunning():
            self.tracker.stop()  # إيقاف الخيط بأمان
        event.accept()
